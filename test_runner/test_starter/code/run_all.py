...
if __name__ == "__main__":
    ...
    for lang in langs:
        with open(fr"..\..\dataset\target\all\test.{suffix[lang]}","r",encoding="utf-8") as f:
            codes = []
            for line in f:
                codes.append(line)
        codes_levels = []
        ...
        for level, codes_level in enumerate(codes_levels):
            level += 1
            path = fr"..\..\dataset\target\l{level}"
            if not os.path.exists(path):
                os.makedirs(path)
            with open(fr"..\..\dataset\target\l{level}\test.{suffix[lang]}","w",encoding="utf-8") as f:
    ...
    for level in levels:
        for lang in langs:
            ...
            params += ["--test_file",fr"..\..\dataset\target\{level}\test.{suffix[lang]}"]
            params += ["--out_name", lang]
